/**
 * products.ts — Base de datos completa de productos Biotiza
 * 47 productos · 5 líneas · 3 marcas · Interfaz Product de src/types/index.ts
 */

import type { Product } from '@/types'

// ─────────────────────────────────────────────────────────────────────────
// LÍNEA ORGÁNICOS (9 productos) · #22b573 verde
// ─────────────────────────────────────────────────────────────────────────

const ORGANICOS: Product[] = [
  {
    id: 'bp-calcio',
    slug: 'bp-calcio',
    name: 'BP Calcio',
    full_name: 'BP Calcio — Calcio Orgánico Quelatado',
    line: 'organicos',
    brand: 'bioproductos',
    category: 'Macronutrientes',
    description: 'Fuente orgánica de calcio de alta biodisponibilidad para fortalecer la pared celular, prevenir el bitter pit en manzana y blossom end rot en tomate.',
    technical_description: 'Formulación líquida de calcio orgánico quelatado con ácidos orgánicos de bajo peso molecular que garantizan absorción foliar y radicular de alta eficiencia. Ideal para cultivos con alta demanda de calcio en etapas de engorde y maduración.',
    composition: [
      { ingredient: 'Óxido de Calcio (CaO)',   value: '16 %',  unit: '%' },
    ],
    presentations: [
      { size: '1 L',  unit: 'L' },
      { size: '20 L', unit: 'L' },
    ],
    certifications: ['COFEPRIS', 'OMRI', 'Hecho en México'],
    application_methods: ['foliar', 'fertirrigacion', 'drench'],
    recommended_dose: {
      foliar:        '2–3 mL/L',
      fertirrigacion:'2–4 L/ha',
      drench:        '3–5 L/ha',
    },
    frequency: 'Cada 7–10 días en etapa de engorde y maduración',
    best_stages: ['desarrollo de fruto', 'maduración', 'cuajado'],
    benefits: [
      'Previene bitter pit y blossom end rot',
      'Fortalece la pared celular del fruto',
      'Mejora vida de anaquel y firmeza',
      'Alta biodisponibilidad por quelación orgánica',
    ],
    crops: ['tomate', 'tomate-cherry', 'fresa', 'arandano', 'frambuesa', 'aguacate', 'chile', 'brocoli', 'apio', 'frijol', 'cebolla', 'citricos', 'flores', 'lechuga', 'mango', 'manzana', 'papa', 'papaya', 'pepino', 'pina', 'uva'],
    compatibility: {
      compatible:   ['bp-magnesio', 'bp-potasio', 'bp-nutri', 'n-ultra'],
      incompatible: ['bp-boro', 'biotiza-calcio-boro'],
      conditional:  ['bp-mol'],
    },
    solves_problems: ['deficiencia de calcio', 'bitter pit', 'blossom end rot', 'frutos blandos', 'rajaduras de fruto'],
    featured: true,
  },
  {
    id: 'bp-magnesio',
    slug: 'bp-magnesio',
    name: 'BP Magnesio',
    full_name: 'BP Magnesio — Magnesio Orgánico Quelatado',
    line: 'organicos',
    brand: 'bioproductos',
    category: 'Macronutrientes',
    description: 'Magnesio orgánico quelatado para corregir clorosis intervenal, activar la fotosíntesis y potenciar la absorción de fósforo.',
    technical_description: 'El magnesio es el átomo central de la clorofila y cofactor de más de 300 enzimas. Esta formulación líquida quelada con ácidos orgánicos garantiza asimilación foliar rápida y corrección de deficiencias en 5–7 días.',
    composition: [
      { ingredient: 'Óxido de Magnesio (MgO)', value: '16 %',  unit: '%' },
    ],
    presentations: [
      { size: '1 L',  unit: 'L' },
      { size: '20 L', unit: 'L' },
    ],
    certifications: ['COFEPRIS', 'OMRI', 'Hecho en México'],
    application_methods: ['foliar', 'fertirrigacion'],
    recommended_dose: {
      foliar:        '1.5–2.5 mL/L',
      fertirrigacion:'2–3 L/ha',
    },
    frequency: 'Cada 10–15 días o al detectar clorosis intervenal',
    best_stages: ['crecimiento vegetativo', 'floración', 'desarrollo de fruto'],
    benefits: [
      'Corrige clorosis intervenal rápidamente',
      'Activa la síntesis de clorofila',
      'Mejora la translocación de fósforo',
      'Aumenta la calidad del fruto',
    ],
    crops: ['tomate', 'tomate-cherry', 'chile', 'citricos', 'aguacate', 'fresa', 'brocoli', 'apio', 'cana-azucar'],
    compatibility: {
      compatible:   [ 'bp-calcio', 'bp-potasio', 'n-ultra' ],
      incompatible: [],
      conditional:  ['bp-boro'],
    },
    solves_problems: ['deficiencia de magnesio', 'clorosis intervenal', 'hojas amarillas', 'baja fotosíntesis'],
    featured: false,
  },
  {
    id: 'bp-potasio',
    slug: 'bp-potasio',
    name: 'BP Potasio',
    full_name: 'BP Potasio — Potasio Orgánico Quelatado',
    line: 'organicos',
    brand: 'bioproductos',
    category: 'Macronutrientes',
    description: 'Potasio orgánico de alta concentración para activar la síntesis de azúcares, mejorar calibre y brix, y fortalecer la resistencia al estrés hídrico.',
    technical_description: 'El potasio regula la apertura y cierre estomático, la translocación de fotosintatos y la síntesis de proteínas. Fundamental en la etapa de llenado y maduración de frutos.',
    composition: [
      { ingredient: 'Óxido de Potasio (K₂O)',  value: '16 %',  unit: '%' },
    ],
    presentations: [
      { size: '1 L',  unit: 'L' },
      { size: '20 L', unit: 'L' },
    ],
    certifications: ['COFEPRIS', 'OMRI', 'Hecho en México'],
    application_methods: ['foliar', 'fertirrigacion'],
    recommended_dose: {
      foliar:        '2–3 mL/L',
      fertirrigacion:'3–5 L/ha',
    },
    frequency: 'Cada 10 días en etapa de engorde y maduración',
    best_stages: ['desarrollo de fruto', 'maduración', 'cuajado'],
    benefits: [
      'Aumenta grados Brix (contenido de azúcares)',
      'Mejora calibre y peso del fruto',
      'Fortalece resistencia al estrés hídrico',
      'Mejora la coloración del fruto',
    ],
    crops: ['tomate', 'tomate-cherry', 'fresa', 'arandano', 'citricos', 'chile', 'brocoli', 'apio', 'maiz', 'cana-azucar', 'frijol', 'agave', 'aguacate', 'arroz', 'cebolla', 'esparrago', 'flores', 'manzana', 'nuez', 'papa', 'pina', 'platano', 'sorgo', 'trigo', 'uva'],
    compatibility: {
      compatible:   ['bp-calcio', 'bp-magnesio', 'bp-gross', 'k-ultra'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['deficiencia de potasio', 'bajo calibre', 'bajos brix', 'frutos insípidos', 'quemaduras de borde en hojas'],
    featured: true,
  },
  {
    id: 'bp-koren',
    slug: 'bp-koren',
    name: 'BP Koren',
    full_name: 'BP Koren — Enraizador de Alta Concentración',
    line: 'organicos',
    brand: 'bioproductos',
    category: 'Enraizadores',
    description: 'Enraizador de máxima concentración (5000 ppm AIB) para transplantes exigentes y recuperación de sistemas radiculares dañados.',
    composition: [
      { ingredient: 'Ácido Indolbutírico (AIB)', value: '5000 ppm', unit: 'ppm' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'OMRI', 'Hecho en México'],
    application_methods: ['drench', 'fertirrigacion'],
    recommended_dose: {
      drench:        '1–1.5 mL/L',
      fertirrigacion:'0.5–1 L/ha',
    },
    frequency: 'Al trasplante y en recuperación; máx 2 veces por ciclo',
    best_stages: ['trasplante', 'emergencia'],
    benefits: [
      'La mayor concentración de AIB de la línea',
      'Rescate de plantas con daño radicular',
      'Ideal para trasplantes de alta inversión (berries)',
      'Estimula raíces absorbentes en 5–7 días',
    ],
    crops: ['arandano', 'frambuesa', 'zarzamora', 'aguacate', 'fresa', 'agave', 'alfalfa', 'apio', 'arroz', 'brocoli', 'calabacin', 'calabaza', 'cana-azucar', 'cebolla', 'chile', 'esparrago', 'flores', 'frijol', 'lechuga', 'manzana', 'nuez', 'papa', 'papaya', 'pepino', 'pina', 'platano', 'sandia', 'sorgo', 'tomate', 'tomate-cherry', 'uva'],
    compatibility: {
      compatible:   [ 'biotiza-lactobacillus' ],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['daño radicular', 'trasplante difícil', 'plantas débiles', 'raíces podridas por exceso de riego'],
    featured: true,
  },
  {
    id: 'bp-nitro-fx',
    slug: 'bp-nitro-fx',
    name: 'BP Nitro FX',
    full_name: 'BP Nitro FX — Nitrógeno Orgánico + Aminoácidos',
    line: 'organicos',
    brand: 'bioproductos',
    category: 'Nitrógeno orgánico',
    description: 'Fuente de nitrógeno orgánico con alto contenido de aminoácidos libres. Activa el metabolismo de la planta y promueve el crecimiento vegetativo de forma natural.',
    composition: [
      { ingredient: 'Aminoácidos libres',    value: '16 %',  unit: '%' },
      { ingredient: 'Nitrógeno total (N)',    value: '2.7 %', unit: '%' },
      { ingredient: 'Pentóxido de fósforo (P₂O₅)', value: '2.5 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }, { size: '20 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'OMRI', 'Hecho en México'],
    application_methods: ['foliar', 'fertirrigacion'],
    recommended_dose: {
      foliar:        '1.5–2 mL/L',
      fertirrigacion:'2–3 L/ha',
    },
    frequency: 'Cada 10–14 días en etapa vegetativa',
    best_stages: ['crecimiento vegetativo', 'trasplante', 'inducción floral'],
    benefits: [
      'Aminoácidos de asimilación directa sin gasto energético',
      'Estimula el crecimiento vegetativo rápido',
      'Activa enzimas clave del metabolismo',
      'Compatible con programas orgánicos certificados',
    ],
    crops: ['tomate', 'chile', 'fresa', 'arandano', 'frambuesa'],
    compatibility: {
      compatible:   [ 'bp-koren', 'bp-nutri' ],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['lento crecimiento vegetativo', 'plantas débiles', 'deficiencia de nitrógeno orgánico'],
    featured: false,
  },
  {
    id: 'bp-oxyagro',
    slug: 'bp-oxyagro',
    name: 'BP Oxyagro',
    full_name: 'BP Oxyagro — Peróxido de Hidrógeno Agrícola',
    line: 'organicos',
    brand: 'bioproductos',
    category: 'Sanitizantes de suelo',
    description: 'Peróxido de hidrógeno al 35% de grado agrícola para desinfección de suelo, control de Pythium, Phytophthora y Fusarium, y oxigenación de la zona radicular.',
    composition: [
      { ingredient: 'Peróxido de Hidrógeno (H₂O₂)', value: '35 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }, { size: '20 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['drench', 'fertirrigacion'],
    recommended_dose: {
      drench:        '1–2 mL/L (preventivo); 3–5 mL/L (curativo)',
      fertirrigacion:'3–5 L/ha',
    },
    frequency: 'Cada 15–21 días como preventivo; cada 7 días en tratamiento curativo',
    best_stages: ['pre-siembra', 'trasplante', 'crecimiento vegetativo'],
    benefits: [
      'Oxigena la zona radicular',
      'Controla Pythium, Phytophthora y Fusarium',
      'Desinfecta sistemas de riego',
      'No deja residuos en suelo ni planta',
    ],
    crops: ['tomate', 'chile', 'fresa', 'arandano', 'frambuesa', 'zarzamora', 'aguacate', 'citricos'],
    compatibility: {
      compatible:   [],
      incompatible: [ 'biotiza-lactobacillus', 'bp-koren' ],
      conditional:  [],
    },
    solves_problems: ['dampimg-off', 'Pythium', 'Phytophthora', 'Fusarium radicular', 'suelo asfixiante'],
    featured: false,
  },
  {
    id: 'bp-cu-agro',
    slug: 'bp-cu-agro',
    name: 'BP Cu Agro Organic',
    full_name: 'BP Cu Agro Organic — Cobre Orgánico Quelatado',
    line: 'organicos',
    brand: 'bioproductos',
    category: 'Micronutrientes',
    description: 'Cobre orgánico quelado para corrección de deficiencias y como fungicida-bactericida preventivo de origen orgánico certificado.',
    composition: [
      { ingredient: 'Óxido de Cobre (CuO)', value: '16 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }, { size: '20 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'OMRI', 'Hecho en México'],
    application_methods: ['foliar', 'fertirrigacion'],
    recommended_dose: {
      foliar:        '0.5–1 mL/L',
      fertirrigacion:'0.5–1 L/ha',
    },
    frequency: 'Cada 14–21 días como preventivo',
    best_stages: ['crecimiento vegetativo', 'floración', 'desarrollo de fruto'],
    benefits: [
      'Doble función: nutrición y protección',
      'Acción preventiva contra hongos y bacterias',
      'Quelación orgánica de alta absorción',
      'Compatible con programas OMRI',
    ],
    crops: ['tomate', 'tomate-cherry', 'chile', 'aguacate', 'citricos', 'arandano', 'brocoli'],
    compatibility: {
      compatible:   ['bp-calcio', 'bp-magnesio', 'zen-fungi'],
      incompatible: ['zen-cu'],
      conditional:  [],
    },
    solves_problems: ['deficiencia de cobre', 'mildiu velloso', 'bacteriosis', 'antracnosis'],
    featured: false,
  },
  {
    id: 'bp-nutri',
    slug: 'bp-nutri',
    name: 'BP Nutri',
    full_name: 'BP Nutri — Algas Marinas + NPK Orgánico',
    line: 'organicos',
    brand: 'bioproductos',
    category: 'Bioestimulantes orgánicos',
    description: 'Extracto de algas marinas (Ascophyllum nodosum) enriquecido con NPK orgánico para estimulación del crecimiento, resistencia al estrés y mejora de la calidad del fruto.',
    composition: [
      { ingredient: 'Extracto de algas marinas', value: '25 %',  unit: '%' },
      { ingredient: 'Nitrógeno total (N)',         value: '0.5 %', unit: '%' },
      { ingredient: 'Pentóxido de fósforo (P₂O₅)', value: '0.5 %', unit: '%' },
      { ingredient: 'Óxido de Potasio (K₂O)',       value: '5.3 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }, { size: '20 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'OMRI', 'Hecho en México'],
    application_methods: ['foliar', 'fertirrigacion', 'drench'],
    recommended_dose: {
      foliar:        '1.5–2 mL/L',
      fertirrigacion:'1.5–2.5 L/ha',
      drench:        '2 L/ha',
    },
    frequency: 'Cada 10–14 días en todo el ciclo',
    best_stages: ['crecimiento vegetativo', 'inducción floral', 'floración', 'desarrollo de fruto'],
    benefits: [
      'Estimula el crecimiento vegetativo y radicular',
      'Aumenta la resistencia al estrés hídrico y térmico',
      'Aporta citoquininas, auxinas y giberelinas naturales',
      'Mejora calidad y uniformidad del fruto',
    ],
    crops: ['tomate', 'tomate-cherry', 'fresa', 'arandano', 'chile', 'aguacate', 'citricos', 'brocoli', 'apio', 'cana-azucar', 'agave', 'alfalfa', 'arroz', 'calabacin', 'cebolla', 'esparrago', 'flores', 'frambuesa', 'lechuga', 'mango', 'manzana', 'nuez', 'pepino', 'pina', 'platano', 'sorgo', 'trigo'],
    compatibility: {
      compatible:   [ 'bp-calcio', 'bp-koren', 'bp-nitro-fx' ],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['estrés hídrico', 'estrés térmico', 'lento crecimiento', 'plantas estresadas'],
    featured: false,
  },
  {
    id: 'ae-calcium',
    slug: 'ae-calcium',
    name: 'AE Calcium',
    full_name: 'AE Calcium — Calcio Premium con Ácidos Polihidroxicarboxílicos',
    line: 'organicos',
    brand: 'bioproductos',
    category: 'Macronutrientes premium',
    description: 'Calcio de máxima biodisponibilidad formulado con ácidos polihidroxicarboxílicos y ácidos húmicos/fúlvicos para una absorción foliar excepcional y efecto mejorador de suelo simultáneo.',
    composition: [
      { ingredient: 'Óxido de Calcio (CaO)',                   value: '16 %',  unit: '%' },
      { ingredient: 'Ácidos Polihidroxicarboxílicos',           value: '15 %',  unit: '%' },
      { ingredient: 'Ácidos Fúlvicos y Húmicos',               value: '10 %',  unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }, { size: '20 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'OMRI', 'Hecho en México'],
    application_methods: ['foliar', 'fertirrigacion'],
    recommended_dose: {
      foliar:        '2–3 mL/L',
      fertirrigacion:'2–4 L/ha',
    },
    frequency: 'Cada 7–10 días en etapa crítica de calcio',
    best_stages: ['desarrollo de fruto', 'maduración'],
    benefits: [
      'Triple acción: calcio + ácidos polihidroxicarboxílicos + húmicos',
      'Máxima absorción foliar de calcio',
      'Mejora estructura del suelo y CIC',
      'Ideal donde la calidad premium es exigida por el mercado de exportación',
    ],
    crops: ['tomate', 'tomate-cherry', 'fresa', 'arandano', 'frambuesa', 'aguacate', 'brocoli', 'apio', 'calabacin', 'calabaza', 'cebolla', 'chile', 'lechuga', 'mango', 'manzana', 'nuez', 'papa', 'pepino', 'pina', 'platano', 'sandia', 'uva', 'zarzamora'],
    compatibility: {
      compatible:   [ 'bp-magnesio', 'bp-potasio' ],
      incompatible: ['bp-calcio', 'bp-boro'],
      conditional:  [],
    },
    solves_problems: ['deficiencia severa de calcio', 'baja CIC del suelo', 'frutos con baja firmeza'],
    featured: true,
  },
]

// ─────────────────────────────────────────────────────────────────────────
// LÍNEA ESPECIALIDADES (6 productos) · #0e6e99 teal
// ─────────────────────────────────────────────────────────────────────────

const ESPECIALIDADES: Product[] = [
  {
    id: 'bp-mol',
    slug: 'bp-mol',
    name: 'BP Mol',
    full_name: 'BP Mol — Molibdeno + Boro + Fósforo (Polvo)',
    line: 'especialidades',
    brand: 'bioproductos',
    category: 'Micronutrientes',
    description: 'Corrector en polvo soluble de molibdeno y boro con soporte de fósforo y potasio. Esencial para la fijación de nitrógeno, síntesis de enzimas nitrato-reductasa y desarrollo de la floración.',
    composition: [
      { ingredient: 'Pentóxido de fósforo (P₂O₅)', value: '8.7 %', unit: '%' },
      { ingredient: 'Óxido de Potasio (K₂O)',        value: '5.8 %', unit: '%' },
      { ingredient: 'Boro (B)',                       value: '6.7 %', unit: '%' },
      { ingredient: 'Molibdeno (Mo)',                 value: '1.0 %', unit: '%' },
    ],
    presentations: [{ size: '1 kg', unit: 'kg' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['foliar', 'fertirrigacion'],
    recommended_dose: {
      foliar:        '0.5–1 g/L',
      fertirrigacion:'0.5–1 kg/ha',
    },
    frequency: 'Cada 15–21 días o en corrección puntual',
    best_stages: ['inducción floral', 'floración', 'cuajado'],
    benefits: [
      'Activa la enzima nitrato-reductasa',
      'Mejora la fijación de nitrógeno en leguminosas',
      'El boro favorece la germinación del pólen',
      'Corrección rápida de deficiencias múltiples',
    ],
    crops: ['tomate', 'tomate-cherry', 'chile', 'aguacate', 'fresa', 'citricos', 'brocoli', 'maiz', 'frijol', 'agave', 'mango', 'papa'],
    compatibility: {
      compatible:   ['bp-vit', 'bp-cuaje', 'n-ultra'],
      incompatible: [],
      conditional:  ['bp-calcio'],
    },
    solves_problems: ['deficiencia de molibdeno', 'deficiencia de boro', 'baja germinación de pólen', 'mala fijación de N'],
    featured: false,
  },
  {
    id: 'bp-boro',
    slug: 'bp-boro',
    name: 'BP Boro',
    full_name: 'BP Boro — Boro Líquido de Alta Concentración',
    line: 'especialidades',
    brand: 'bioproductos',
    category: 'Micronutrientes',
    description: 'Boro líquido de alta concentración con nitrógeno vehiculizador para máxima movilidad en la planta. Fundamental en floración, cuajado y desarrollo de la pared celular.',
    composition: [
      { ingredient: 'Boro (B)',              value: '16 %',  unit: '%' },
      { ingredient: 'Nitrógeno total (N)',    value: '1.3 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }, { size: '20 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['foliar', 'fertirrigacion'],
    recommended_dose: {
      foliar:        '0.3–0.5 mL/L',
      fertirrigacion:'0.3–0.5 L/ha',
    },
    frequency: 'Inicio de floración y repetir en plena flor; máx 3 veces/ciclo',
    best_stages: ['inducción floral', 'floración'],
    benefits: [
      'Mejora la germinación del tubo polínico',
      'Aumenta el cuaje de frutos',
      'Fortalece la pared celular',
      'Previene corcho interno en manzana y aguacate',
    ],
    crops: ['tomate', 'tomate-cherry', 'aguacate', 'citricos', 'chile', 'fresa', 'brocoli', 'apio', 'maiz', 'frijol', 'alfalfa', 'arandano', 'frambuesa', 'mango', 'manzana', 'nuez', 'pepino', 'pina', 'platano', 'sandia', 'sorgo', 'trigo', 'uva'],
    compatibility: {
      compatible:   ['bp-cuaje', 'bp-fiore', 'bp-vit'],
      incompatible: ['bp-calcio', 'ae-calcium'],
      conditional:  [],
    },
    solves_problems: ['deficiencia de boro', 'caída de flores', 'bajo cuaje', 'corcho interno', 'deformación de frutos'],
    featured: false,
  },
  {
    id: 'bp-vit',
    slug: 'bp-vit',
    name: 'BP Vit',
    full_name: 'BP Vit — Vitaminas + Molibdeno + Cisteína',
    line: 'especialidades',
    brand: 'bioproductos',
    category: 'Bioestimulantes vitamínicos',
    description: 'Complejo vitamínico agrícola con molibdeno, L-cisteína y ácido fólico para activar el metabolismo enzimático, mejorar la resistencia al estrés y estimular la floración.',
    composition: [
      { ingredient: 'Nitrógeno total (N)',           value: '1.3 %', unit: '%' },
      { ingredient: 'Pentóxido de fósforo (P₂O₅)',   value: '2.2 %', unit: '%' },
      { ingredient: 'Molibdeno (Mo)',                 value: '0.6 %', unit: '%' },
      { ingredient: 'L-Cisteína + Ácido Fólico',     value: '5.1 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['foliar'],
    recommended_dose: {
      foliar: '1–1.5 mL/L',
    },
    frequency: 'Cada 10–14 días en periodos de estrés o en pre-floración',
    best_stages: ['inducción floral', 'floración', 'crecimiento vegetativo'],
    benefits: [
      'Activa más de 100 enzimas metabólicas',
      'Reduce el estrés por calor, frío y déficit hídrico',
      'Mejora la eficiencia en la absorción de nutrientes',
      'Estimula el metabolismo energético de la planta',
    ],
    crops: ['tomate', 'tomate-cherry', 'chile', 'fresa', 'arandano', 'citricos', 'brocoli', 'aguacate', 'cebolla'],
    compatibility: {
      compatible:   ['bp-boro', 'bp-mol', 'bp-cuaje'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['estrés abiótico', 'mala respuesta a fertirrigación', 'plantas apagadas'],
    featured: false,
  },
  {
    id: 'biotiza-coadyuvante',
    slug: 'biotiza-coadyuvante',
    name: 'Biotiza Coadyuvante',
    full_name: 'Biotiza Coadyuvante — Aceite Parafinado Agrícola',
    line: 'especialidades',
    brand: 'bioproductos',
    category: 'Coadyuvantes',
    description: 'Aceite parafinado de alta pureza como coadyuvante-surfactante para mejorar la adherencia, penetración y distribución de cualquier agroquímico o biosolución.',
    composition: [
      { ingredient: 'Aceite parafinado', value: '88 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }, { size: '20 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['foliar'],
    recommended_dose: {
      foliar: '0.5–1 mL/L (adicionar al caldo)',
    },
    frequency: 'En cada aplicación foliar donde se requiera mejor adherencia',
    best_stages: ['crecimiento vegetativo', 'floración', 'desarrollo de fruto'],
    benefits: [
      'Mejora la adhesión de agroquímicos hasta 3×',
      'Aumenta la penetración cuticular',
      'Compatible con la mayoría de biosoluciones',
      'Efecto anti-evaporación del caldo',
    ],
    crops: ['tomate', 'chile', 'fresa', 'arandano', 'aguacate', 'citricos', 'lechuga'],
    compatibility: {
      compatible:   ['zen-chrys', 'zen-can', 'zen-fungi', 'bp-calcio', 'bp-magnesio'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['baja adherencia foliar', 'ineficiencia de productos foliares', 'lavado por lluvia'],
    featured: false,
  },
  {
    id: 'biotiza-lactobacillus',
    slug: 'biotiza-lactobacillus',
    name: 'Biotiza Lactobacillus',
    full_name: 'Biotiza Lactobacillus — Inoculante Probiótico Agrícola',
    line: 'especialidades',
    brand: 'bioproductos',
    category: 'Inoculantes microbianos',
    description: 'Inoculante probiótico con dos cepas de Lactobacillus de alta concentración para mejorar la microbiota del suelo, competir con patógenos y solubilizar fósforo y zinc.',
    composition: [
      { ingredient: 'Lactobacillus paracasei',  value: '56 000 000 UFC/g', unit: 'UFC/g' },
      { ingredient: 'Lactobacillus plantarum',  value: '88 000 000 UFC/g', unit: 'UFC/g' },
    ],
    presentations: [{ size: '1 kg', unit: 'kg' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['drench', 'fertirrigacion'],
    recommended_dose: {
      drench:        '1–2 g/L de solución',
      fertirrigacion:'0.5–1 kg/ha',
    },
    frequency: 'Cada 15–21 días; iniciar al trasplante',
    best_stages: ['pre-siembra', 'trasplante', 'crecimiento vegetativo'],
    benefits: [
      'Mejora la microbiota benéfica del suelo',
      'Compite con Fusarium, Pythium y bacterias patógenas',
      'Solubiliza fósforo y zinc no disponibles',
      'Mejora la estructura y salud del suelo a largo plazo',
    ],
    crops: ['tomate', 'tomate-cherry', 'chile', 'fresa', 'arandano', 'frambuesa', 'zarzamora', 'brocoli', 'apio', 'maiz', 'cana-azucar', 'frijol', 'agave', 'alfalfa', 'flores', 'pepino', 'pina', 'platano'],
    compatibility: {
      compatible:   [ 'bp-koren', 'bp-nutri' ],
      incompatible: ['bp-oxyagro', 'zen-cu'],
      conditional:  [],
    },
    solves_problems: ['suelo biológicamente deprimido', 'dependencia de fungicidas químicos', 'baja disponibilidad de P y Zn'],
    featured: false,
  },
  {
    id: 'biotiza-calcio-boro',
    slug: 'biotiza-calcio-boro',
    name: 'Biotiza Calcio Boro',
    full_name: 'Biotiza Calcio Boro — Calcio + Boro Premium',
    line: 'especialidades',
    brand: 'bioproductos',
    category: 'Macronutrientes + Micronutrientes',
    description: 'Formulación combinada de calcio y boro en una sola aplicación. Optimiza la translocación de calcio al fruto y activa simultáneamente la germinación del tubo polínico.',
    composition: [
      { ingredient: 'Calcio (Ca)',  value: '16 %', unit: '%' },
      { ingredient: 'Boro (B)',     value: '9 %',  unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }, { size: '20 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['foliar', 'fertirrigacion'],
    recommended_dose: {
      foliar:        '1.5–2.5 mL/L',
      fertirrigacion:'2–3 L/ha',
    },
    frequency: 'Inicio de floración, plena flor y inicio de cuaje',
    best_stages: ['floración', 'cuajado', 'desarrollo de fruto'],
    benefits: [
      'Doble acción en una sola aplicación',
      'Reduce la caída de flores y aumenta cuaje',
      'Previene blossom end rot y corcho interno',
      'Optimiza la logística de aplicaciones',
    ],
    crops: ['tomate', 'tomate-cherry', 'aguacate', 'citricos', 'chile', 'fresa', 'calabacin', 'calabaza', 'lechuga', 'papaya', 'sandia'],
    compatibility: {
      compatible:   ['bp-potasio', 'bp-gross'],
      incompatible: ['bp-calcio', 'bp-boro', 'ae-calcium'],
      conditional:  [],
    },
    solves_problems: ['caída de flores', 'bajo cuaje', 'deficiencia simultánea de Ca y B'],
    featured: false,
  },
]

// ─────────────────────────────────────────────────────────────────────────
// LÍNEA BIOESTIMULANTES (6 productos) · #e8690f naranja
// ─────────────────────────────────────────────────────────────────────────

const BIOESTIMULANTES: Product[] = [
  {
    id: 'bp-fiore',
    slug: 'bp-fiore',
    name: 'BP Fioré',
    full_name: 'BP Fioré — Inductor de Floración (Polvo)',
    line: 'bioestimulantes',
    brand: 'bioproductos',
    category: 'Inductores de floración',
    description: 'Bioestimulante de floración en polvo con auxinas naturales, aminoácidos y azufre. Homogeneiza la apertura floral, aumenta la densidad de flores y mejora la receptividad del estigma.',
    composition: [
      { ingredient: 'Auxinas naturales',  value: '18.2 %', unit: '%' },
      { ingredient: 'Aminoácidos libres', value: '16 %',   unit: '%' },
      { ingredient: 'Azufre (S)',          value: '12 %',   unit: '%' },
    ],
    presentations: [{ size: '1 kg', unit: 'kg' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['foliar'],
    recommended_dose: {
      foliar: '0.5–1 g/L (aplicar en preflora y flor)',
    },
    frequency: '3–5 días antes del inicio de floración; repetir en plena flor',
    best_stages: ['inducción floral', 'floración'],
    benefits: [
      'Homogeneiza la apertura floral en toda la parcela',
      'Aumenta la densidad de yemas florales',
      'Mejora la receptividad del estigma y germinación del pólen',
      'El azufre actúa como fungistático natural',
    ],
    crops: ['tomate', 'tomate-cherry', 'fresa', 'chile', 'aguacate', 'arandano', 'frambuesa', 'zarzamora', 'maiz', 'frijol', 'calabacin', 'calabaza', 'citricos', 'flores', 'mango', 'manzana', 'papaya', 'pepino', 'pina'],
    compatibility: {
      compatible:   ['bp-boro', 'bp-cuaje', 'bp-vit'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['floración irregular', 'baja densidad de flores', 'mala cuaja', 'flores no viables'],
    featured: true,
  },
  {
    id: 'bp-gibb',
    slug: 'bp-gibb',
    name: 'BP Gibb',
    full_name: 'BP Gibb — Giberelina AG3 10%',
    line: 'bioestimulantes',
    brand: 'bioproductos',
    category: 'Reguladores de crecimiento',
    description: 'Giberelina AG3 de alta pureza al 10% para elongación de entrenudos, ruptura de dormancia, inducción de partenocarpia y aumento de calibre en uva y bayas.',
    composition: [
      { ingredient: 'Ácido Giberélico (AG3)', value: '10 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['foliar'],
    recommended_dose: {
      foliar: '0.2–0.5 mL/L (calibrar según cultivo)',
    },
    frequency: 'Según fenología del cultivo; ver protocolo específico',
    best_stages: ['pre-siembra', 'crecimiento vegetativo', 'inducción floral', 'desarrollo de fruto'],
    benefits: [
      'Promueve la elongación del raquis en uva',
      'Induce partenocarpia en tomate y pepino',
      'Aumenta calibre de bayas',
      'Rompe dormancia en semillas y yemas',
    ],
    crops: ['tomate', 'fresa', 'arandano', 'frambuesa', 'zarzamora', 'aguacate'],
    compatibility: {
      compatible:   ['bp-cuaje', 'bp-gross'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['calibre pequeño de bayas', 'dormancia prolongada', 'partenocarpia requerida', 'raquis corto'],
    featured: false,
  },
  {
    id: 'bp-cuaje',
    slug: 'bp-cuaje',
    name: 'BP Cuaje',
    full_name: 'BP Cuaje — Bioestimulante de Cuajado de Frutos',
    line: 'bioestimulantes',
    brand: 'bioproductos',
    category: 'Cuajado de frutos',
    description: 'Bioestimulante de cuajado con NPK equilibrado, molibdeno y cofactores enzimáticos. Reduce el aborto floral y aumenta el porcentaje de cuaje en condiciones adversas.',
    composition: [
      { ingredient: 'Nitrógeno total (N)',           value: '0.6 %', unit: '%' },
      { ingredient: 'Pentóxido de fósforo (P₂O₅)',   value: '2.5 %', unit: '%' },
      { ingredient: 'Óxido de Potasio (K₂O)',         value: '2.5 %', unit: '%' },
      { ingredient: 'Molibdeno (Mo)',                 value: '0.7 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }, { size: '20 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['foliar'],
    recommended_dose: {
      foliar: '1.5–2.5 mL/L en plena flor y cuaje',
    },
    frequency: 'En plena flor, a los 5 días y a los 10 días del cuaje',
    best_stages: ['floración', 'cuajado'],
    benefits: [
      'Aumenta el porcentaje de cuaje hasta 30%',
      'Reduce el aborto floral por estrés térmico',
      'El molibdeno optimiza la enzima nitrato-reductasa',
      'Mejora la uniformidad del calibre del lote',
    ],
    crops: ['tomate', 'tomate-cherry', 'chile', 'fresa', 'aguacate', 'arandano', 'maiz', 'frijol', 'calabacin', 'calabaza', 'citricos', 'frambuesa', 'mango', 'pepino', 'sandia', 'uva', 'zarzamora'],
    compatibility: {
      compatible:   ['bp-fiore', 'bp-boro', 'bp-vit', 'bp-gibb'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['aborto floral', 'bajo porcentaje de cuaje', 'caída de frutos recién cuajados', 'temperatura adversa en floración'],
    featured: false,
  },
  {
    id: 'bp-gross',
    slug: 'bp-gross',
    name: 'BP Gross',
    full_name: 'BP Gross — Engordador de Fruto (Polvo)',
    line: 'bioestimulantes',
    brand: 'bioproductos',
    category: 'Engorde de fruto',
    description: 'Engordador de fruto en polvo con boro, molibdeno, potasio y expansinas vegetales. Promueve la división y expansión celular del fruto para maximizar calibre y peso.',
    composition: [
      { ingredient: 'Boro (B)',           value: '13.8 %', unit: '%' },
      { ingredient: 'Molibdeno (Mo)',     value: '5.2 %',  unit: '%' },
      { ingredient: 'Óxido de K (K₂O)',  value: '5.1 %',  unit: '%' },
      { ingredient: 'Expansinas',        value: '1.7 %',  unit: '%' },
    ],
    presentations: [{ size: '1 kg', unit: 'kg' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['foliar'],
    recommended_dose: {
      foliar: '0.5–1 g/L al inicio del engorde y a los 7 días',
    },
    frequency: '2 aplicaciones en inicio de engorde; cada 10 días si la demanda es alta',
    best_stages: ['cuajado', 'desarrollo de fruto'],
    benefits: [
      'Las expansinas aceleran la elongación celular',
      'El boro y K potencian el transporte de azúcares al fruto',
      'Aumenta el calibre comercial hasta una categoría superior',
      'Mejora el peso promedio del fruto',
    ],
    crops: ['tomate', 'tomate-cherry', 'chile', 'fresa', 'arandano', 'frambuesa', 'aguacate', 'brocoli', 'agave', 'calabaza', 'mango', 'manzana', 'pepino', 'pina', 'platano', 'sandia', 'uva', 'zarzamora'],
    compatibility: {
      compatible:   ['bp-potasio', 'bp-cuaje', 'k-ultra'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['calibre insuficiente', 'bajo peso de fruto', 'frutos pequeños por estrés'],
    featured: true,
  },
  {
    id: 'bp-acua',
    slug: 'bp-acua',
    name: 'BP Acua',
    full_name: 'BP Acua — Sanitizante de Agua con Sal Amonio',
    line: 'bioestimulantes',
    brand: 'bioproductos',
    category: 'Sanitizantes',
    description: 'Sanitizante de agua de riego a base de sal de amonio cuaternario al 50% para eliminar bacterias, algas y biofilm en sistemas de riego por goteo.',
    composition: [
      { ingredient: 'Sal de Amonio Cuaternario', value: '50 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['fertirrigacion'],
    recommended_dose: {
      fertirrigacion: '0.5–1 mL/L en agua de riego',
    },
    frequency: 'Cada 15–21 días como preventivo; semanal en limpieza de sistema',
    best_stages: ['crecimiento vegetativo', 'desarrollo de fruto'],
    benefits: [
      'Elimina biofilm en goteros y tuberías',
      'Control de bacterias y algas en agua de riego',
      'Previene obstrucción de goteros',
      'Mejora la uniformidad del riego y la fertirrigación',
    ],
    crops: ['tomate', 'tomate-cherry', 'chile', 'fresa', 'arandano', 'frambuesa', 'zarzamora', 'brocoli', 'apio', 'cana-azucar', 'esparrago'],
    compatibility: {
      compatible:   [],
      incompatible: ['biotiza-lactobacillus'],
      conditional:  [],
    },
    solves_problems: ['biofilm en sistema de riego', 'goteros tapados', 'bacterias en agua de riego', 'algas en tanques'],
    featured: false,
  },
  {
    id: 'bp-fresh',
    slug: 'bp-fresh',
    name: 'BP Fresh',
    full_name: 'BP Fresh — Sanitizante Foliar Premium',
    line: 'bioestimulantes',
    brand: 'bioproductos',
    category: 'Sanitizantes',
    description: 'Sanitizante foliar con sal de amonio y povidona para limpieza de herramientas, superficies y follaje. Reduce la carga bacteriana superficial y previene la diseminación de enfermedades.',
    composition: [
      { ingredient: 'Sal de Amonio Cuaternario', value: '60 %', unit: '%' },
      { ingredient: 'Povidona',                  value: '8 %',  unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['aspersion', 'foliar'],
    recommended_dose: {
      aspersion: '2–3 mL/L en superficies y herramientas',
      foliar:    '1 mL/L preventivo foliar',
    },
    frequency: 'Desinfección de herramientas: en cada uso. Foliar: cada 14 días',
    best_stages: ['crecimiento vegetativo', 'floración', 'post-cosecha'],
    benefits: [
      'Reduce la diseminación de bacterias y hongos en campo',
      'Sanitización de herramientas de poda',
      'La povidona potencia la acción bactericida',
      'Compatible con protocolos de inocuidad alimentaria',
    ],
    crops: ['tomate', 'tomate-cherry', 'chile', 'fresa', 'arandano', 'brocoli', 'apio', 'frijol', 'calabacin', 'calabaza', 'cebolla', 'esparrago', 'flores', 'lechuga', 'papa', 'papaya', 'pina'],
    compatibility: {
      compatible:   [],
      incompatible: ['biotiza-lactobacillus'],
      conditional:  [],
    },
    solves_problems: ['contaminación bacteriana por herramientas', 'alta carga microbiana foliar', 'cancrosis bacteriana'],
    featured: false,
  },
]

// ─────────────────────────────────────────────────────────────────────────
// LÍNEA NUTRICIÓN LÍQUIDA (9 productos) · #f28a3d naranja claro
// ─────────────────────────────────────────────────────────────────────────

const NUTRICION: Product[] = [
  {
    id: 'n-ultra',
    slug: 'n-ultra',
    name: 'N-Ultra',
    full_name: 'N-Ultra — Nitrógeno Líquido 30%',
    line: 'nutricion',
    brand: 'bioproductos',
    category: 'Macronutrientes',
    description: 'Nitrógeno líquido de alta concentración (30% N total) para fertirrigación intensiva. Ideal para etapas de máxima demanda nitrogenada.',
    composition: [
      { ingredient: 'Nitrógeno total (N)', value: '30 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }, { size: '20 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['fertirrigacion'],
    recommended_dose: {
      fertirrigacion: '3–8 L/ha según demanda del cultivo',
    },
    frequency: 'Según programa de fertirrigación y análisis de tejido',
    best_stages: ['crecimiento vegetativo', 'inducción floral'],
    benefits: [
      'Máxima concentración de N en presentación líquida',
      'Fácil mezcla en cabezal de riego',
      'Sin sólidos: no tapa goteros',
      'Compatible con todos los sistemas de fertirrigación',
    ],
    crops: ['tomate', 'tomate-cherry', 'chile', 'fresa', 'arandano', 'frambuesa', 'zarzamora', 'aguacate', 'citricos', 'brocoli', 'apio', 'maiz', 'cana-azucar', 'frijol', 'agave', 'arroz', 'calabacin', 'calabaza', 'cebolla', 'esparrago', 'flores', 'lechuga', 'manzana', 'nuez', 'papa', 'papaya', 'pepino', 'pina', 'platano', 'sandia', 'sorgo', 'trigo', 'uva'],
    compatibility: {
      compatible:   ['p-ultra', 'k-ultra', 'ca-ultra', 'mg-ultra', 'bp-mix'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['deficiencia de nitrógeno', 'lento crecimiento vegetativo', 'hojas pálidas'],
    featured: false,
  },
  {
    id: 'p-ultra',
    slug: 'p-ultra',
    name: 'P-Ultra',
    full_name: 'P-Ultra — Fósforo Líquido P₂O₅ 52%',
    line: 'nutricion',
    brand: 'bioproductos',
    category: 'Macronutrientes',
    description: 'Fuente líquida de fósforo de máxima concentración (52% P₂O₅) para estimular el desarrollo radicular, la floración y la energía celular (ATP).',
    composition: [
      { ingredient: 'Pentóxido de fósforo (P₂O₅)', value: '52 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }, { size: '20 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['fertirrigacion'],
    recommended_dose: {
      fertirrigacion: '1–3 L/ha',
    },
    frequency: 'Alta demanda en trasplante, pre-floración y cuaje',
    best_stages: ['trasplante', 'inducción floral', 'floración', 'cuajado'],
    benefits: [
      'Mayor concentración de P disponible en el mercado',
      'Estimula el desarrollo radicular en trasplante',
      'Activa la energía celular (ATP/ADP)',
      'Mejora la translocación de azúcares',
    ],
    crops: ['tomate', 'tomate-cherry', 'chile', 'fresa', 'arandano', 'frambuesa', 'zarzamora', 'aguacate', 'brocoli', 'apio', 'maiz', 'cana-azucar', 'frijol', 'alfalfa', 'calabacin', 'calabaza', 'cebolla', 'flores', 'papa', 'papaya', 'pepino', 'sandia', 'sorgo', 'trigo'],
    compatibility: {
      compatible:   ['n-ultra', 'k-ultra', 'ca-ultra', 'mg-ultra'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['deficiencia de fósforo', 'coloración púrpura en hojas', 'raíces cortas', 'baja energía celular'],
    featured: false,
  },
  {
    id: 'k-ultra',
    slug: 'k-ultra',
    name: 'K-Ultra',
    full_name: 'K-Ultra — Potasio Líquido K₂O 40%',
    line: 'nutricion',
    brand: 'bioproductos',
    category: 'Macronutrientes',
    description: 'Potasio líquido de alta concentración (40% K₂O) sin cloro para llenado de fruto, grados Brix y resistencia al estrés hídrico.',
    composition: [
      { ingredient: 'Óxido de Potasio (K₂O)', value: '40 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }, { size: '20 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['fertirrigacion'],
    recommended_dose: {
      fertirrigacion: '3–6 L/ha en engorde y maduración',
    },
    frequency: 'Alta demanda en cuaje, engorde y maduración',
    best_stages: ['cuajado', 'desarrollo de fruto', 'maduración'],
    benefits: [
      'Sin cloro: ideal para cultivos sensibles',
      'Aumenta grados Brix y azúcares totales',
      'Mejora color y vida de anaquel',
      'Fortalece la resistencia al estrés hídrico',
    ],
    crops: ['tomate', 'tomate-cherry', 'fresa', 'arandano', 'frambuesa', 'chile', 'citricos', 'brocoli', 'apio', 'maiz', 'cana-azucar', 'frijol', 'agave', 'aguacate', 'alfalfa', 'arroz', 'calabacin', 'calabaza', 'cebolla', 'esparrago', 'flores', 'lechuga', 'mango', 'manzana', 'nuez', 'papa', 'papaya', 'pepino', 'pina', 'platano', 'sandia', 'sorgo', 'trigo', 'uva', 'zarzamora'],
    compatibility: {
      compatible:   ['n-ultra', 'p-ultra', 'ca-ultra', 'mg-ultra', 'bp-mix'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['bajos Brix', 'frutos insípidos', 'baja coloración', 'deficiencia de potasio'],
    featured: true,
  },
  {
    id: 'ca-ultra',
    slug: 'ca-ultra',
    name: 'Ca-Ultra',
    full_name: 'Ca-Ultra — Calcio + Nitrógeno Premium',
    line: 'nutricion',
    brand: 'bioproductos',
    category: 'Macronutrientes',
    description: 'Calcio en solución con nitrógeno vehiculizador para maximizar la translocación de calcio al fruto y tejidos en crecimiento activo.',
    composition: [
      { ingredient: 'Nitrógeno total (N)',  value: '7 %',  unit: '%' },
      { ingredient: 'Calcio (Ca)',          value: '10 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }, { size: '20 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['fertirrigacion', 'foliar'],
    recommended_dose: {
      fertirrigacion: '3–5 L/ha',
      foliar:         '3–4 mL/L',
    },
    frequency: 'Alta demanda en crecimiento activo, cuaje y engorde',
    best_stages: ['crecimiento vegetativo', 'cuajado', 'desarrollo de fruto'],
    benefits: [
      'El N actúa como vehículo para la absorción de Ca',
      'Previene tip burn en lechuga y BER en tomate',
      'Fortalece meristemos y tejidos nuevos',
      'Compatible con todo el programa de fertirrigación',
    ],
    crops: ['tomate', 'tomate-cherry', 'chile', 'fresa', 'arandano', 'frambuesa', 'zarzamora', 'brocoli', 'apio', 'maiz', 'frijol', 'aguacate'],
    compatibility: {
      compatible:   ['n-ultra', 'k-ultra', 'mg-ultra', 'bp-mix'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['deficiencia de calcio', 'blossom end rot', 'tip burn', 'necrosis apical'],
    featured: false,
  },
  {
    id: 'mg-ultra',
    slug: 'mg-ultra',
    name: 'Mg-Ultra',
    full_name: 'Mg-Ultra — Magnesio + Azufre',
    line: 'nutricion',
    brand: 'bioproductos',
    category: 'Macronutrientes secundarios',
    description: 'Fuente líquida de magnesio y azufre para corrección de deficiencias, activación de la fotosíntesis y síntesis de aminoácidos azufrados.',
    composition: [
      { ingredient: 'Magnesio (Mg)',  value: '5 %',   unit: '%' },
      { ingredient: 'Azufre (S)',     value: '5.5 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }, { size: '20 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['fertirrigacion', 'foliar'],
    recommended_dose: {
      fertirrigacion: '2–4 L/ha',
      foliar:         '2–3 mL/L',
    },
    frequency: 'Cada 10–14 días; alta demanda en crecimiento vegetativo y llenado',
    best_stages: ['crecimiento vegetativo', 'floración', 'desarrollo de fruto'],
    benefits: [
      'El Mg es el átomo central de la clorofila',
      'El S es clave en la síntesis de aminoácidos azufrados',
      'Doble corrección en una sola aplicación',
      'Mejora la eficiencia de conversión lumínica',
    ],
    crops: ['tomate', 'tomate-cherry', 'chile', 'fresa', 'arandano', 'aguacate', 'citricos', 'brocoli', 'apio', 'cana-azucar', 'cebolla', 'esparrago', 'lechuga', 'papa', 'pina', 'platano'],
    compatibility: {
      compatible:   ['n-ultra', 'k-ultra', 'ca-ultra', 'bp-mix'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['clorosis intervenal', 'deficiencia de magnesio', 'deficiencia de azufre', 'hojas amarillas entre venas'],
    featured: false,
  },
  {
    id: 'bp-ferrum',
    slug: 'bp-ferrum',
    name: 'BP Ferrum',
    full_name: 'BP Ferrum — Hierro Quelatado EDDHA/EDTA 9%',
    line: 'nutricion',
    brand: 'bioproductos',
    category: 'Micronutrientes',
    description: 'Hierro quelatado de alta estabilidad al 9% para corrección de clorosis férrica en suelos calcáreos o con pH alto. Estable hasta pH 9.',
    composition: [
      { ingredient: 'Hierro quelatado (Fe)', value: '9 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }, { size: '20 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['fertirrigacion', 'foliar'],
    recommended_dose: {
      fertirrigacion: '1–2 L/ha',
      foliar:         '0.5–1 mL/L',
    },
    frequency: 'Cada 14–21 días o al detectar clorosis férrica',
    best_stages: ['crecimiento vegetativo', 'floración'],
    benefits: [
      'Estable en suelos con pH elevado (hasta 9)',
      'Corrección de clorosis férrica en 7–10 días',
      'Compatible con todos los sistemas de fertirrigación',
      'Previene el amarillamiento inter-venal superior',
    ],
    crops: ['tomate', 'fresa', 'arandano', 'citricos', 'aguacate'],
    compatibility: {
      compatible:   ['bp-mix', 'ca-ultra', 'mg-ultra'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['clorosis férrica', 'hojas jóvenes amarillas', 'suelos calcáreos'],
    featured: false,
  },
  {
    id: 'bp-zinc',
    slug: 'bp-zinc',
    name: 'BP Zinc',
    full_name: 'BP Zinc — Zinc Quelatado 8%',
    line: 'nutricion',
    brand: 'bioproductos',
    category: 'Micronutrientes',
    description: 'Zinc quelatado al 8% para activar la síntesis de auxinas, mejorar la elongación celular y prevenir la "hoja pequeña" o roseta en frutales y tomate.',
    composition: [
      { ingredient: 'Zinc quelatado (Zn)', value: '8 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }, { size: '20 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['fertirrigacion', 'foliar'],
    recommended_dose: {
      fertirrigacion: '0.5–1 L/ha',
      foliar:         '0.5 mL/L',
    },
    frequency: 'Cada 14–21 días o al detectar deficiencia',
    best_stages: ['crecimiento vegetativo', 'inducción floral'],
    benefits: [
      'Activa la síntesis de auxinas endógenas',
      'Previene la roseta y hoja pequeña en frutales',
      'Mejora la elongación de entrenudos',
      'Cofactor de más de 300 enzimas',
    ],
    crops: ['tomate', 'tomate-cherry', 'chile', 'aguacate', 'citricos', 'arandano', 'brocoli', 'maiz', 'cana-azucar', 'frijol', 'arroz', 'nuez', 'sorgo', 'trigo'],
    compatibility: {
      compatible:   ['bp-mix', 'bp-ferrum', 'n-ultra'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['deficiencia de zinc', 'hojas pequeñas', 'entrenudos cortos', 'roseta en frutales'],
    featured: false,
  },
  {
    id: 'bp-mix',
    slug: 'bp-mix',
    name: 'BP Mix',
    full_name: 'BP Mix — Paquete Completo de Micronutrientes',
    line: 'nutricion',
    brand: 'bioproductos',
    category: 'Micronutrientes',
    description: 'Solución completa de microelementos quelatados con azufre, boro, cobre, hierro, manganeso, molibdeno y zinc. Una sola aplicación para corregir o prevenir deficiencias múltiples.',
    composition: [
      { ingredient: 'Azufre (S)',        value: '5 %',   unit: '%' },
      { ingredient: 'Boro (B)',           value: '5.5 %', unit: '%' },
      { ingredient: 'Cobre (Cu)',         value: '0.3 %', unit: '%' },
      { ingredient: 'Hierro (Fe)',        value: '7 %',   unit: '%' },
      { ingredient: 'Manganeso (Mn)',     value: '2 %',   unit: '%' },
      { ingredient: 'Molibdeno (Mo)',     value: '0.3 %', unit: '%' },
      { ingredient: 'Zinc (Zn)',          value: '2 %',   unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }, { size: '20 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['fertirrigacion', 'foliar'],
    recommended_dose: {
      fertirrigacion: '1–2 L/ha',
      foliar:         '1–1.5 mL/L',
    },
    frequency: 'Cada 14 días como preventivo en todo el ciclo',
    best_stages: ['crecimiento vegetativo', 'floración', 'desarrollo de fruto'],
    benefits: [
      '7 microelementos en una sola aplicación',
      'Prevención y corrección de deficiencias múltiples',
      'Ahorra tiempo y costo de aplicaciones individuales',
      'Fórmula equilibrada para todo el ciclo del cultivo',
    ],
    crops: ['tomate', 'tomate-cherry', 'chile', 'fresa', 'arandano', 'frambuesa', 'zarzamora', 'aguacate', 'citricos', 'brocoli', 'apio', 'maiz', 'cana-azucar', 'frijol', 'agave', 'alfalfa', 'arroz', 'calabacin', 'calabaza', 'esparrago', 'lechuga', 'papa', 'papaya', 'pepino', 'pina', 'sorgo', 'uva'],
    compatibility: {
      compatible:   ['n-ultra', 'k-ultra', 'ca-ultra', 'mg-ultra'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['deficiencias múltiples de microelementos', 'análisis de suelo con varios bajos', 'suelos empobrecidos'],
    featured: false,
  },
]

// ─────────────────────────────────────────────────────────────────────────
// CATEGORÍA BIOPROTECCIÓN (10 productos) · #1189bf azul
// Bioinsecticidas, biofungicidas y bactericidas — incluye los productos
// históricamente etiquetados como "Zentia" de Bioproductos Agrícolas.
// ─────────────────────────────────────────────────────────────────────────

const BIOPROTECCION: Product[] = [
  {
    id: 'zen-chrys',
    slug: 'zen-chrys',
    name: 'Zen-Chrys',
    full_name: 'Zen-Chrys — Bioinsecticida de Piretrinas Naturales',
    line: 'bioproteccion',
    brand: 'bioproductos',
    category: 'Bioinsecticidas',
    description: 'Bioinsecticida de piretrinas naturales al 2.5% para control de trips, mosca blanca, áfidos y ácaros por contacto e ingestión. Rápida acción y baja residualidad.',
    composition: [
      { ingredient: 'Piretrinas naturales', value: '2.5 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['foliar', 'aspersion'],
    recommended_dose: {
      foliar:    '1.5–2.5 mL/L',
      aspersion: '1.5–2.5 L/ha',
    },
    frequency: 'Cada 5–7 días en presión alta; cada 10–14 días como preventivo',
    best_stages: ['crecimiento vegetativo', 'floración', 'desarrollo de fruto'],
    benefits: [
      'Acción de contacto e ingestión',
      'Sin resistencias reportadas',
      'Baja residualidad: ICP corto',
      'Compatible con liberaciones de depredadores',
    ],
    crops: ['tomate', 'tomate-cherry', 'chile', 'fresa', 'arandano', 'frambuesa', 'zarzamora', 'brocoli', 'frijol', 'aguacate', 'arroz', 'calabacin', 'calabaza', 'cebolla', 'citricos', 'flores', 'pepino', 'sandia'],
    compatibility: {
      compatible:   ['biotiza-coadyuvante', 'zen-can', 'zen-fungi'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['trips', 'mosca blanca', 'áfidos', 'ácaros', 'chinche lygus'],
    featured: false,
  },
  {
    id: 'zen-can',
    slug: 'zen-can',
    name: 'Zen-Can',
    full_name: 'Zen-Can — Bioinsecticida de Canela 25%',
    line: 'bioproteccion',
    brand: 'bioproductos',
    category: 'Bioinsecticidas',
    description: 'Bioinsecticida-funguicida basado en aceite de canela al 25%. Controla trips, araña roja y hongos superficiales (oídio, botrytis) por acción de contacto.',
    composition: [
      { ingredient: 'Aceite esencial de Canela', value: '25 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['foliar'],
    recommended_dose: {
      foliar: '1–2 mL/L',
    },
    frequency: 'Cada 7–10 días; alternar con otros modos de acción',
    best_stages: ['crecimiento vegetativo', 'floración', 'desarrollo de fruto'],
    benefits: [
      'Doble acción: insecticida + funguicida',
      'Completamente de origen natural',
      'Sin síntesis química: apto para producción orgánica',
      'El eugenol del aceite de canela altera la membrana celular de hongos e insectos',
    ],
    crops: ['tomate', 'fresa', 'arandano', 'chile', 'frambuesa', 'zarzamora'],
    compatibility: {
      compatible:   ['zen-chrys', 'biotiza-coadyuvante'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['trips', 'araña roja', 'oídio', 'botrytis superficial', 'áfidos'],
    featured: false,
  },
  {
    id: 'max-kill-plus',
    slug: 'max-kill-plus',
    name: 'Max-Kill Plus',
    full_name: 'Max-Kill Plus — Bioinsecticida Multiacción',
    line: 'bioproteccion',
    brand: 'bioproductos',
    category: 'Bioinsecticidas',
    description: 'Bioinsecticida de amplio espectro con sinergia de 5 activos naturales: piretro, ajo, canela, neem y mostaza. Para presiones altas y rotación de modos de acción.',
    composition: [
      { ingredient: 'Piretro natural',      value: 'presente', unit: '' },
      { ingredient: 'Aceite de Ajo',        value: 'presente', unit: '' },
      { ingredient: 'Aceite de Canela',     value: 'presente', unit: '' },
      { ingredient: 'Azadiractina (Neem)',  value: 'presente', unit: '' },
      { ingredient: 'Aceite de Mostaza',    value: 'presente', unit: '' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['foliar'],
    recommended_dose: {
      foliar: '2–3 mL/L en presión alta',
    },
    frequency: 'Cada 5–7 días en picos de infestación; máx 3 aplicaciones consecutivas',
    best_stages: ['crecimiento vegetativo', 'desarrollo de fruto'],
    benefits: [
      'Cinco modos de acción simultáneos previenen resistencias',
      'Amplio espectro: insectos masticadores, chupadores y ácaros',
      'La azadiractina actúa como IGR (regulador de crecimiento de insectos)',
      'Ideal para rotaciones en programas de MIP',
    ],
    crops: ['tomate', 'tomate-cherry', 'chile', 'fresa', 'arandano', 'frambuesa', 'zarzamora', 'brocoli', 'apio', 'maiz', 'cana-azucar', 'frijol', 'agave', 'alfalfa', 'mango', 'papa', 'pina', 'platano', 'sorgo', 'sandia', 'calabaza', 'calabacin'],
    compatibility: {
      compatible:   ['biotiza-coadyuvante'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['plagas con resistencia a insecticidas', 'presión alta de trips y ácaros', 'control de emergencia'],
    featured: true,
  },
  {
    id: 'zen-spider',
    slug: 'zen-spider',
    name: 'Zen-Spider',
    full_name: 'Zen-Spider — Acaricida Biológico D-Limoneno',
    line: 'bioproteccion',
    brand: 'bioproductos',
    category: 'Acaricidas biológicos',
    description: 'Acaricida biológico a base de D-Limoneno (extracto de cítricos) al 2.80% para control de araña roja, araña bimaculada y ácaro del bronceado.',
    composition: [
      { ingredient: 'D-Limoneno (extracto de cítricos)', value: '2.80 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['foliar'],
    recommended_dose: {
      foliar: '1.5–2.5 mL/L; cubrir envés de hoja',
    },
    frequency: 'Cada 5–7 días en ciclos activos; 3 aplicaciones mínimas',
    best_stages: ['crecimiento vegetativo', 'desarrollo de fruto'],
    benefits: [
      'El D-Limoneno disuelve la cera cuticular del ácaro',
      'Activo contra huevo, ninfa y adulto',
      'Sin efectos negativos en fauna benéfica establecida',
      'No deja residuos en fruta',
    ],
    crops: ['tomate', 'fresa', 'chile', 'arandano', 'aguacate', 'citricos', 'papaya', 'uva'],
    compatibility: {
      compatible:   ['zen-chrys', 'biotiza-coadyuvante'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['araña roja', 'Tetranychus urticae', 'ácaro del bronceado', 'Panonychus ulmi'],
    featured: false,
  },
  {
    id: 'zen-cu',
    slug: 'zen-cu',
    name: 'Zen-Cu',
    full_name: 'Zen-Cu — Fungicida-Bactericida de Gluconato de Cobre',
    line: 'bioproteccion',
    brand: 'bioproductos',
    category: 'Fungicidas biológicos',
    description: 'Fungicida y bactericida de cobre quelado con gluconato para máxima adherencia foliar y acción sistémica local. Compatible con programas de residuos cero.',
    composition: [
      { ingredient: 'Gluconato de Cobre', value: '26 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['foliar'],
    recommended_dose: {
      foliar: '1–2 mL/L preventivo; 2–3 mL/L curativo',
    },
    frequency: 'Cada 10–14 días preventivo; cada 7 días curativo',
    best_stages: ['crecimiento vegetativo', 'floración', 'desarrollo de fruto'],
    benefits: [
      'El gluconato mejora la adherencia y penetración cuticular',
      'Espectro amplio: hongos y bacterias',
      'Sin residuos en fruta en dosis preventiva',
      'Compatible con protocolos Primus/GlobalG.A.P.',
    ],
    crops: ['tomate', 'tomate-cherry', 'chile', 'aguacate', 'citricos', 'arandano', 'brocoli', 'esparrago'],
    compatibility: {
      compatible:   ['zen-fungi', 'biotiza-coadyuvante'],
      incompatible: ['bp-cu-agro', 'biotiza-lactobacillus'],
      conditional:  [],
    },
    solves_problems: ['mildiu velloso', 'bacteriosis', 'antracnosis', 'cancro bacteriano'],
    featured: false,
  },
  {
    id: 'healex',
    slug: 'healex',
    name: 'Healex',
    full_name: 'Healex — Cicatrizante y Fungicida de Terpenos de Toronja',
    line: 'bioproteccion',
    brand: 'bioproductos',
    category: 'Cicatrizantes-fungicidas',
    description: 'Terpenos de extracto de semilla de toronja al 95% para cicatrización de heridas de poda, control de botrytis en cicatrices y desinfección de tejidos dañados.',
    composition: [
      { ingredient: 'Terpenos de Toronja (extracto semilla)', value: '95 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['aspersion', 'foliar'],
    recommended_dose: {
      aspersion: '1–2 mL/L en heridas y zonas de poda',
      foliar:    '1 mL/L preventivo',
    },
    frequency: 'Aplicar inmediatamente post-poda; repetir a los 5 días',
    best_stages: ['post-cosecha', 'crecimiento vegetativo'],
    benefits: [
      'Sella heridas de poda en 24–48 h',
      'Previene la entrada de Botrytis y Sclerotinia por heridas',
      'Los terpenos actúan como antiséptico natural',
      'Sin fitotoxicidad en dosis recomendada',
    ],
    crops: ['arandano', 'frambuesa', 'zarzamora', 'fresa', 'aguacate', 'tomate', 'tomate-cherry'],
    compatibility: {
      compatible:   ['biotiza-coadyuvante'],
      incompatible: ['zen-fungi'],
      conditional:  [],
    },
    solves_problems: ['botrytis en heridas de poda', 'cicatrización lenta', 'infecciones post-poda'],
    featured: false,
  },
  {
    id: 'zen-fungi',
    slug: 'zen-fungi',
    name: 'Zen-Fungi',
    full_name: 'Zen-Fungi — Biofungicida de Gobernadora',
    line: 'bioproteccion',
    brand: 'bioproductos',
    category: 'Biofungicidas',
    description: 'Biofungicida de extracto de gobernadora (Larrea tridentata) al 90% con potente acción contra hongos fitopatógenos de hoja y fruto.',
    composition: [
      { ingredient: 'Extracto de Gobernadora (Larrea tridentata)', value: '90 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['foliar'],
    recommended_dose: {
      foliar: '1.5–2.5 mL/L',
    },
    frequency: 'Cada 7–10 días en presión de enfermedad; cada 14 días preventivo',
    best_stages: ['crecimiento vegetativo', 'floración', 'desarrollo de fruto'],
    benefits: [
      'Activo contra Botrytis, Oídio, Alternaria y Antracnosis',
      'Los NDGA de la gobernadora inhiben la germinación de esporas',
      'Compatible con liberaciones de abejorros y abejas',
      'No genera resistencias',
    ],
    crops: ['fresa', 'arandano', 'frambuesa', 'zarzamora', 'tomate', 'tomate-cherry', 'chile', 'apio', 'arroz', 'calabacin', 'calabaza', 'esparrago', 'flores', 'lechuga', 'mango', 'manzana', 'nuez', 'papa', 'platano', 'sandia', 'trigo', 'uva'],
    compatibility: {
      compatible:   ['zen-cu', 'zen-chrys', 'biotiza-coadyuvante', 'bp-cu-agro'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['botrytis', 'oídio', 'alternaria', 'antracnosis', 'Podosphaera'],
    featured: false,
  },
  {
    id: 'funbac-plus',
    slug: 'funbac-plus',
    name: 'Funbac Plus',
    full_name: 'Funbac Plus — Fungicida-Bactericida de Aceites Esenciales',
    line: 'bioproteccion',
    brand: 'bioproductos',
    category: 'Fungicidas-bactericidas biológicos',
    description: 'Biofungicida y bactericida sinérgico con aceites esenciales de té, cedro y eucalipto para control preventivo y curativo de hongos y bacterias de hoja y fruto.',
    composition: [
      { ingredient: 'Aceite esencial de Árbol de Té', value: '5 %', unit: '%' },
      { ingredient: 'Aceite esencial de Cedro',       value: '2 %', unit: '%' },
      { ingredient: 'Aceite esencial de Eucalipto',   value: '1 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['foliar'],
    recommended_dose: {
      foliar: '1.5–2 mL/L',
    },
    frequency: 'Cada 7–10 días en presión alta; alternar con Zen-Fungi',
    best_stages: ['crecimiento vegetativo', 'floración', 'desarrollo de fruto'],
    benefits: [
      'Sinergia de tres aceites esenciales con modos de acción distintos',
      'Control de bacterias gram+ y gram-',
      'El aceite de árbol de té actúa contra el biofilm bacteriano',
      'Refuerza el efecto de los cúpricos',
    ],
    crops: ['tomate', 'tomate-cherry', 'chile', 'fresa', 'arandano', 'aguacate', 'maiz', 'cana-azucar', 'frijol', 'cebolla', 'lechuga', 'papa', 'pepino', 'zarzamora'],
    compatibility: {
      compatible:   ['zen-cu', 'zen-fungi', 'biotiza-coadyuvante'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['Pseudomonas', 'Xanthomonas', 'Cladosporium', 'Alternaria', 'bacteriosis foliar'],
    featured: false,
  },
  {
    id: 'nemapro',
    slug: 'nemapro',
    name: 'Nemapro',
    full_name: 'Nemapro — Control Biológico de Nematodos',
    line: 'bioproteccion',
    brand: 'bioproductos',
    category: 'Control de nematodos',
    description: 'Producto biológico para manejo integrado de nematodos fitoparásitos en suelo. Formulación en desarrollo — contactar a Biotiza para disponibilidad y ficha técnica completa.',
    composition: [
      { ingredient: 'Activos biológicos nematicidas', value: 'ver ficha técnica', unit: '' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['drench'],
    recommended_dose: {
      drench: 'Consultar ficha técnica',
    },
    frequency: 'Según protocolo agronómico específico',
    best_stages: ['pre-siembra', 'trasplante'],
    benefits: [
      'Control biológico de nematodos fitoparásitos',
      'Compatible con otros microorganismos benéficos',
      'Sin impacto en fauna edáfica benéfica',
    ],
    crops: ['tomate', 'chile', 'fresa', 'arandano', 'platano'],
    compatibility: {
      compatible:   [ 'biotiza-lactobacillus' ],
      incompatible: ['bp-oxyagro'],
      conditional:  [],
    },
    solves_problems: ['Meloidogyne spp.', 'nematodos de nódulos', 'daño radicular por nematodos'],
    featured: false,
  },
  {
    id: 'enhancer',
    slug: 'enhancer',
    name: 'Enhancer',
    full_name: 'Enhancer — Potenciador de Insecticidas con Sulfonatos',
    line: 'bioproteccion',
    brand: 'bioproductos',
    category: 'Potenciadores',
    description: 'Potenciador de insecticidas y fungicidas a base de sulfonatos al 26% para aumentar la eficiencia de los productos de bioprotección hasta 3×.',
    composition: [
      { ingredient: 'Sulfonatos', value: '26 %', unit: '%' },
    ],
    presentations: [{ size: '1 L', unit: 'L' }],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['foliar'],
    recommended_dose: {
      foliar: '0.5–1 mL/L (adicionar al caldo insecticida o fungicida)',
    },
    frequency: 'En cada aplicación de bioprotección donde se requiera mayor eficiencia',
    best_stages: ['crecimiento vegetativo', 'floración', 'desarrollo de fruto'],
    benefits: [
      'Aumenta la eficiencia de insecticidas y fungicidas hasta 3×',
      'Mejora la penetración cuticular de los activos',
      'Reduce el pH del caldo de aspersión (optimiza estabilidad)',
      'Ahorro de hasta 30% en dosis de producto activo',
    ],
    crops: ['tomate', 'chile', 'fresa', 'arandano', 'frambuesa', 'zarzamora'],
    compatibility: {
      compatible:   ['zen-chrys', 'zen-can', 'max-kill-plus', 'zen-cu', 'zen-fungi', 'funbac-plus'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['baja eficiencia de insecticidas', 'plagas con cutícula gruesa', 'agua de pH alto en mezcla'],
    featured: false,
  },
]

// ─────────────────────────────────────────────────────────────────────────
// MARCA AGROBIONSA (7 productos) · Control biológico microbiano
// ─────────────────────────────────────────────────────────────────────────

const AGROBIONSA: Product[] = [
  {
    id: 'agb-lilac',
    slug: 'lilac',
    name: 'LILAC®',
    full_name: 'LILAC® — Bionematicida a base de Paecilomyces lilacinus',
    line: 'bioproteccion',
    brand: 'agrobionsa',
    category: 'Bionematicidas',
    description: 'Hongo nematófago que parasita huevos y hembras de nemátodos fitoparásitos. Reduce la eclosión y el parasitismo de Meloidogyne en cultivos de exportación, sin residuos y sin intervalo de seguridad.',
    technical_description: 'Polvo humectable con 1.2 × 10¹² conidios de Paecilomyces lilacinus en 240 g. Actúa produciendo toxinas y parasitando huevos y hembras de nemátodos agalladores en suelos ácidos. Registro COFEPRIS: RSCO-NEMA-0907-X0011-002-3.0.',
    composition: [
      { ingredient: 'Paecilomyces lilacinus', value: '1.2 × 10¹² conidios / 240 g', unit: 'conidios' },
    ],
    presentations: [
      { size: '240 g', unit: 'g' },
    ],
    certifications: ['COFEPRIS', 'OMRI', 'Hecho en México'],
    application_methods: ['drench', 'fertirrigacion'],
    recommended_dose: {
      drench:         '240–480 g/ha',
      fertirrigacion: '240–480 g/ha',
    },
    frequency: '2 aplicaciones a intervalo de 14–28 días según cultivo',
    best_stages: ['trasplante', 'pre-siembra', 'crecimiento vegetativo'],
    benefits: [
      'Parasita huevos y hembras de nemátodos',
      'Sin intervalo de seguridad · Sin residuos',
      'Compatible con programas orgánicos OMRI',
      'Efecto curativo y preventivo',
    ],
    crops: ['tomate', 'chile', 'fresa', 'arandano', 'frambuesa', 'zarzamora'],
    compatibility: {
      compatible:   [ 'agb-elicitor-sin', 'bp-koren' ],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['Meloidogyne incognita', 'nematodos agalladores', 'nódulos radiculares'],
    featured: false,
  },
  {
    id: 'agb-elicitor-sin',
    slug: 'elicitor-sin',
    name: 'ELICITOR-SIN®',
    full_name: 'ELICITOR-SIN® — Biofungicida a base de Trichoderma harzianum',
    line: 'bioproteccion',
    brand: 'agrobionsa',
    category: 'Biofungicidas',
    description: 'Trichoderma harzianum que forma una barrera de micoparasitismo alrededor de la raíz, previniendo damping-off y hongos fitopatógenos del suelo mediante competencia y antibiosis.',
    technical_description: 'Polvo humectable con 1.2 × 10¹² conidios de Trichoderma harzianum en 240 g. Modo de acción: competencia, micoparasitismo y producción de enzimas antibióticas. Registro COFEPRIS: RSCO-FUNG-0301B-0287-002-3.0.',
    composition: [
      { ingredient: 'Trichoderma harzianum', value: '1.2 × 10¹² conidios / 240 g', unit: 'conidios' },
    ],
    presentations: [
      { size: '240 g', unit: 'g' },
    ],
    certifications: ['COFEPRIS', 'OMRI', 'Hecho en México'],
    application_methods: ['drench', 'fertirrigacion'],
    recommended_dose: {
      drench:         '240–480 g/ha',
      fertirrigacion: '240–480 g/ha',
    },
    frequency: '2–3 aplicaciones a intervalos de 7–11 días',
    best_stages: ['trasplante', 'pre-siembra', 'crecimiento vegetativo'],
    benefits: [
      'Previene damping-off y Fusarium',
      'Coloniza rizósfera · barrera biológica',
      'Estimula defensas naturales de la planta',
      'OMRI · compatible con orgánicos',
    ],
    crops: ['tomate', 'tomate-cherry', 'chile', 'fresa', 'arandano', 'frambuesa', 'zarzamora', 'brocoli', 'apio', 'agave', 'cebolla', 'esparrago', 'lechuga', 'papa', 'papaya', 'pina'],
    compatibility: {
      compatible:   [ 'agb-lilac', 'bp-koren' ],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['damping-off', 'Fusarium sp.', 'Phytophthora sp.', 'marchitez vascular'],
    featured: true,
  },
  {
    id: 'agb-entomotri',
    slug: 'entomotri',
    name: 'ENTOMOTRI®',
    full_name: 'ENTOMOTRI® — Bioinsecticida multi-cepa contra Paratrioza',
    line: 'bioproteccion',
    brand: 'agrobionsa',
    category: 'Bioinsecticidas',
    description: 'Combinación sinérgica de tres hongos entomopatógenos (Metarhizium, Beauveria y Paecilomyces) para control efectivo de paratrioza (Bactericera cockerelli) en solanáceas.',
    technical_description: 'Polvo humectable con 2.4 × 10¹² conidios totales en 240 g de tres especies: Metarhizium anisopliae, Beauveria bassiana y Paecilomyces fumosoroseus. Registro COFEPRIS: RSCO-INAC-0908-0285-002-5.7.',
    composition: [
      { ingredient: 'Metarhizium + Beauveria + Paecilomyces', value: '2.4 × 10¹² conidios / 240 g', unit: 'conidios' },
    ],
    presentations: [
      { size: '240 g', unit: 'g' },
    ],
    certifications: ['COFEPRIS', 'OMRI', 'Hecho en México'],
    application_methods: ['foliar'],
    recommended_dose: {
      foliar: '360–480 g / 200 L de agua',
    },
    frequency: '2 aplicaciones foliares a intervalos de 5 días',
    best_stages: ['crecimiento vegetativo', 'floración', 'desarrollo de fruto'],
    benefits: [
      'Triple modo de acción sinérgico',
      'Control específico de paratrioza',
      'Sin residuos · Sin intervalo de seguridad',
      'Apto programas orgánicos OMRI',
    ],
    crops: ['tomate', 'chile', 'tomate-cherry'],
    compatibility: {
      compatible:   ['agb-beauver', 'agb-paecilom', 'funbac-plus'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['paratrioza', 'Bactericera cockerelli', 'punta morada'],
    featured: false,
  },
  {
    id: 'agb-paecilom',
    slug: 'paecilom',
    name: 'PAECILOM®',
    full_name: 'PAECILOM® — Bioinsecticida a base de Paecilomyces fumosoroseus',
    line: 'bioproteccion',
    brand: 'agrobionsa',
    category: 'Bioinsecticidas',
    description: 'Hongo entomopatógeno que causa mortalidad en todos los estados de desarrollo de la mosquita blanca (Bemisia tabaci), incluyendo huevos, ninfas y adultos.',
    technical_description: 'Polvo humectable con 1.2 × 10¹² conidios de Paecilomyces fumosoroseus en 240 g. Registro COFEPRIS: RSCO-INAC-0906-0286-002-3.0.',
    composition: [
      { ingredient: 'Paecilomyces fumosoroseus', value: '1.2 × 10¹² conidios / 240 g', unit: 'conidios' },
    ],
    presentations: [
      { size: '240 g', unit: 'g' },
    ],
    certifications: ['COFEPRIS', 'OMRI', 'Hecho en México'],
    application_methods: ['foliar'],
    recommended_dose: {
      foliar: '240–480 g / 200 L de agua',
    },
    frequency: '2 aplicaciones foliares a intervalos de 7 días',
    best_stages: ['crecimiento vegetativo', 'floración', 'desarrollo de fruto'],
    benefits: [
      'Control en huevos, ninfas y adultos',
      'Sin residuos · Sin PHI',
      'OMRI · apto orgánico',
      'Compatible con fauna benéfica',
    ],
    crops: ['tomate', 'chile', 'fresa', 'arandano', 'frambuesa', 'zarzamora', 'citricos'],
    compatibility: {
      compatible:   ['agb-beauver', 'agb-entomotri', 'funbac-plus'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['mosquita blanca', 'Bemisia tabaci', 'ninfas de mosca blanca'],
    featured: false,
  },
  {
    id: 'agb-dextrux',
    slug: 'dextrux',
    name: 'DEXTRUX®',
    full_name: 'DEXTRUX® — Bioinsecticida a base de Metarhizium anisopliae',
    line: 'bioproteccion',
    brand: 'agrobionsa',
    category: 'Bioinsecticidas',
    description: 'Metarhizium anisopliae formulado para control específico del picudo del chile (Anthonomus eugenii). Las conidias penetran la cutícula y provocan la momificación del insecto.',
    technical_description: 'Polvo humectable con 1.2 × 10¹² conidios de Metarhizium anisopliae en 240 g. Registro COFEPRIS: RSCO-INAC-0376-0250-315-3.0.',
    composition: [
      { ingredient: 'Metarhizium anisopliae', value: '1.2 × 10¹² conidios / 240 g', unit: 'conidios' },
    ],
    presentations: [
      { size: '240 g', unit: 'g' },
    ],
    certifications: ['COFEPRIS', 'OMRI', 'Hecho en México'],
    application_methods: ['foliar'],
    recommended_dose: {
      foliar: '360–480 g / 200 L de agua',
    },
    frequency: '2 aplicaciones foliares a intervalos de 5 días',
    best_stages: ['floración', 'cuajado', 'desarrollo de fruto'],
    benefits: [
      'Control específico del picudo del chile',
      'Penetración cuticular directa',
      'OMRI · residuo cero',
      'Seguro para polinizadores',
    ],
    crops: ['chile'],
    compatibility: {
      compatible:   ['agb-beauver', 'agb-entomotri'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['picudo del chile', 'Anthonomus eugenii'],
    featured: false,
  },
  {
    id: 'agb-beauver',
    slug: 'beauver',
    name: 'BEAUVER®',
    full_name: 'BEAUVER® — Bioinsecticida a base de Beauveria bassiana',
    line: 'bioproteccion',
    brand: 'agrobionsa',
    category: 'Bioinsecticidas',
    description: 'Beauveria bassiana que infecta insectos plaga a través del intestino, espiráculos y especialmente por penetración directa del integumento — independiente de los hábitos alimenticios.',
    technical_description: 'Polvo humectable con 1.28 × 10¹² conidios de Beauveria bassiana en 240 g. Registro COFEPRIS: RSCO-INAC-0903-0195-0291-002-2.5.',
    composition: [
      { ingredient: 'Beauveria bassiana', value: '1.28 × 10¹² conidios / 240 g', unit: 'conidios' },
    ],
    presentations: [
      { size: '240 g', unit: 'g' },
    ],
    certifications: ['COFEPRIS', 'OMRI', 'Hecho en México'],
    application_methods: ['foliar'],
    recommended_dose: {
      foliar: '480 g / 200 L de agua',
    },
    frequency: '2 aplicaciones foliares a intervalos de 14 días',
    best_stages: ['crecimiento vegetativo', 'floración', 'desarrollo de fruto'],
    benefits: [
      'Control amplio de insectos chupadores',
      'Penetración por integumento',
      'OMRI · orgánico certificado',
      'Compatible con programas IPM',
    ],
    crops: ['tomate', 'chile', 'fresa', 'arandano', 'frambuesa', 'zarzamora', 'agave', 'pepino', 'pina'],
    compatibility: {
      compatible:   ['agb-paecilom', 'agb-entomotri', 'agb-dextrux'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['mosquita blanca', 'Bemisia tabaci', 'thrips', 'pulgones'],
    featured: false,
  },
  {
    id: 'agb-lecanii-green',
    slug: 'lecanii-green',
    name: 'LECANII GREEN®',
    full_name: 'LECANII GREEN® — Bioinsecticida líquido a base de Verticillium lecanii',
    line: 'bioproteccion',
    brand: 'agrobionsa',
    category: 'Bioinsecticidas',
    description: 'Formulación líquida de Verticillium lecanii que infecta pulgones (Myzus persicae) en cualquier estado de desarrollo. Fácil aplicación foliar directa sin rehidratación.',
    technical_description: 'Suspensión líquida con 1.2 × 10¹² conidios de Verticillium lecanii por litro. Registro COFEPRIS: RSCO-INAC-0104M-0237-315-2.5.',
    composition: [
      { ingredient: 'Verticillium lecanii', value: '1.2 × 10¹² conidios / L', unit: 'conidios' },
    ],
    presentations: [
      { size: '1 L', unit: 'L' },
    ],
    certifications: ['COFEPRIS', 'Hecho en México'],
    application_methods: ['foliar'],
    recommended_dose: {
      foliar: '1.0–2.0 L/ha',
    },
    frequency: '2 aplicaciones foliares a intervalos de 7 días',
    best_stages: ['crecimiento vegetativo', 'floración', 'desarrollo de fruto'],
    benefits: [
      'Control específico de pulgón',
      'Formulación líquida lista para usar',
      'Sin residuos · Sin intervalo de seguridad',
      'Compatible con polinizadores',
    ],
    crops: ['tomate', 'chile', 'nuez', 'papaya', 'sorgo', 'trigo', 'citricos'],
    compatibility: {
      compatible:   ['agb-beauver', 'agb-paecilom', 'funbac-plus'],
      incompatible: [],
      conditional:  [],
    },
    solves_problems: ['pulgón', 'Myzus persicae', 'áfidos'],
    featured: false,
  },
]

// ─────────────────────────────────────────────────────────────────────────
// MARCA VEGANIC (1 producto) · Biosoluciones premium desde España
// ─────────────────────────────────────────────────────────────────────────

const VEGANIC: Product[] = [
  {
    id: 'vgn-brotanic',
    slug: 'brotanic',
    name: 'BROTANIC',
    full_name: 'BROTANIC — Reactivador de Brotación y Flujo de Savia',
    line: 'nutricion',
    brand: 'veganic',
    category: 'Fisionutriente · Bioestimulante de brotación',
    description: 'Reactivador de la brotación que estimula la formación de nuevos haces vasculares dentro de la planta, acelerando el flujo de savia por xilema y floema. Al mover más agua y nutrientes de forma natural logra una brotación vigorosa y uniforme en trasplantes y después de podas — sin depender de hormonas. Aplicable en todas las etapas: como genera nuevas vías de transporte más rápidas, mejora también la asimilación y translocación de todo lo que se aplica (nutrientes, fungicidas e insecticidas).',
    technical_description: 'Fisionutriente líquido a base de Cobre, Manganeso y Zinc (pH 2,73, densidad 1,14 g/cm³, aspecto marrón). Su acción no es solo nutricional: actúa como vasodilatador de los haces vasculares y fluidificante de savia, promoviendo la diferenciación de nuevo tejido conductor y una translocación sistémica ascendente y descendente más rápida. El soporte fisiológico es la triada de microelementos: el Cu interviene en la lignificación y formación de los vasos del xilema, el Zn es precursor de las auxinas endógenas (de ahí que la brotación se logre sin hormonas exógenas) y el Mn activa la fotosíntesis y numerosas enzimas. Esta reactivación de savia también ayuda a la planta a recuperarse de pudriciones de raíz y cuello. Registro sanitario COFEPRIS RSCO-335/VII/25. Compatible con fertirrigación y aspersión foliar; aplicar temprano por la mañana o por la tarde evitando temperaturas superiores a 35 °C.',
    composition: [
      { ingredient: 'Cobre (Cu)',     value: '1,66 %', unit: '% p/v', notes: 'Lignificación y formación de vasos del xilema' },
      { ingredient: 'Manganeso (Mn)', value: '0,90 %', unit: '% p/v', notes: 'Activación enzimática y fotosíntesis' },
      { ingredient: 'Zinc (Zn)',      value: '0,59 %', unit: '% p/v', notes: 'Precursor de auxinas endógenas (brotación sin hormonas)' },
      { ingredient: 'pH',             value: '2,73',   unit: '—',     notes: 'Formulación ácida · facilita asimilación foliar' },
      { ingredient: 'Densidad',       value: '1,14',   unit: 'g/cm³'                                                       },
    ],
    presentations: [
      { size: '1 L',     unit: 'L' },
      { size: '5 L',     unit: 'L' },
      { size: '20 L',    unit: 'L' },
      { size: '200 L',   unit: 'L' },
      { size: '1 000 L', unit: 'L' },
    ],
    certifications: ['COFEPRIS'],
    application_methods: ['foliar', 'fertirrigacion'],
    recommended_dose: {
      foliar:         '0,5 – 2,5 L / 200 L de agua (según cultivo)',
      fertirrigacion: '1,0 – 3,0 L/ha (según cultivo)',
    },
    frequency: 'En trasplante y después de cada poda; reforzar con aplicaciones escalonadas durante el ciclo (dosis creciente)',
    best_stages: ['trasplante', 'pre-siembra', 'crecimiento vegetativo', 'inducción floral', 'floración', 'desarrollo de fruto'],
    benefits: [
      'Reactiva la brotación de forma natural, sin hormonas',
      'Genera nuevos haces vasculares: más flujo de savia (xilema/floema)',
      'Ideal en trasplante y para recuperar la planta tras la poda',
      'Mejor translocación: nutrientes y fitosanitarios se asimilan más',
      'Acción sistémica ascendente y descendente (raíz ↔ parte aérea)',
      'Ayuda a recuperar plantas con pudrición de raíz o cuello',
      'Certificado Vegan & Cruelty Free · COFEPRIS RSCO-335/VII/25',
    ],
    crops: [
      'tomate', 'tomate-cherry', 'chile', 'brocoli', 'apio', 'pepino', 'lechuga', 'calabacin',
      'fresa', 'arandano', 'frambuesa', 'zarzamora', 'aguacate', 'citricos',
      'maiz', 'cana-azucar', 'frijol', 'agave',
      'manzana', 'uva', 'nuez', 'mango', 'platano', 'papaya', 'sandia',
    ],
    compatibility: {
      compatible:   ['bp-nutri', 'bp-mix', 'n-ultra', 'p-ultra', 'k-ultra'],
      incompatible: ['bp-calcio', 'ca-ultra'],
      conditional:  [],
    },
    solves_problems: [
      'brotación lenta o despareja',
      'estrés de trasplante',
      'recuperación post-poda',
      'savia espesa / mala translocación',
      'baja asimilación de nutrientes y fitosanitarios',
      'dependencia de hormonas para brotar',
      'plantas débiles con pudrición de raíz o cuello',
    ],
    featured: true,
    datasheet_url: '/docs/fichas-tecnicas/brotanic.pdf',
    updated_at: '2025-08-13',
    dosage_table: [
      {
        system:    'Hortalizas y ornamentales',
        dose:      'Foliar 1,0 → 1,5 → 2,0 L / 200 L de agua · Riego 1,0 → 2,0 → 3,0 L/ha',
        method:    'Foliar o fertirrigación',
        frequency: 'Tres aplicaciones escalonadas durante el desarrollo vegetativo y reproductivo',
      },
      {
        system:    'Frutales y tropicales',
        dose:      'Foliar 1,5 → 2,0 → 2,5 L / 200 L de agua · Riego 2,0 → 2,5 → 3,0 L/ha',
        method:    'Foliar o fertirrigación',
        frequency: 'Tres aplicaciones escalonadas durante el desarrollo vegetativo y reproductivo',
      },
      {
        system:    'Granos y cereales',
        dose:      'Foliar 0,5 → 1,0 → 1,5 L / 200 L de agua · Riego 1,0 → 1,5 → 2,0 L/ha',
        method:    'Foliar o fertirrigación',
        frequency: 'Tres aplicaciones escalonadas durante el desarrollo vegetativo y reproductivo',
      },
    ],
    dosage_by_crop: [
      { crop: 'tomate',        cropName: 'Tomate',          stages: ['crecimiento vegetativo', 'floración', 'cuajado'],       dose: '1,0 – 2,0 L/200 L foliar · 1,0 – 3,0 L/ha riego', method: 'Foliar / Fertirrigación', frequency: '3 aplicaciones escalonadas', notes: 'Grupo hortalizas · dosis creciente' },
      { crop: 'tomate-cherry', cropName: 'Tomate Cherry',   stages: ['crecimiento vegetativo', 'floración', 'cuajado'],       dose: '1,0 – 2,0 L/200 L foliar · 1,0 – 3,0 L/ha riego', method: 'Foliar / Fertirrigación', frequency: '3 aplicaciones escalonadas', notes: 'Grupo hortalizas · dosis creciente' },
      { crop: 'chile',         cropName: 'Chile',           stages: ['crecimiento vegetativo', 'floración', 'cuajado'],       dose: '1,0 – 2,0 L/200 L foliar · 1,0 – 3,0 L/ha riego', method: 'Foliar / Fertirrigación', frequency: '3 aplicaciones escalonadas', notes: 'Grupo hortalizas · dosis creciente' },
      { crop: 'brocoli',       cropName: 'Brócoli',         stages: ['crecimiento vegetativo', 'inducción floral'],           dose: '1,0 – 2,0 L/200 L foliar · 1,0 – 3,0 L/ha riego', method: 'Foliar / Fertirrigación', frequency: '3 aplicaciones escalonadas', notes: 'Grupo hortalizas · dosis creciente' },
      { crop: 'apio',          cropName: 'Apio',            stages: ['crecimiento vegetativo'],                               dose: '1,0 – 2,0 L/200 L foliar · 1,0 – 3,0 L/ha riego', method: 'Foliar / Fertirrigación', frequency: '3 aplicaciones escalonadas', notes: 'Grupo hortalizas · dosis creciente' },
      { crop: 'fresa',         cropName: 'Fresa',           stages: ['crecimiento vegetativo', 'floración', 'fructificación'],dose: '1,5 – 2,5 L/200 L foliar · 2,0 – 3,0 L/ha riego', method: 'Foliar / Fertirrigación', frequency: '3 aplicaciones escalonadas', notes: 'Grupo frutales · dosis creciente' },
      { crop: 'arandano',      cropName: 'Arándano',        stages: ['crecimiento vegetativo', 'floración', 'fructificación'],dose: '1,5 – 2,5 L/200 L foliar · 2,0 – 3,0 L/ha riego', method: 'Foliar / Fertirrigación', frequency: '3 aplicaciones escalonadas', notes: 'Grupo frutales · dosis creciente' },
      { crop: 'frambuesa',     cropName: 'Frambuesa',       stages: ['crecimiento vegetativo', 'floración', 'fructificación'],dose: '1,5 – 2,5 L/200 L foliar · 2,0 – 3,0 L/ha riego', method: 'Foliar / Fertirrigación', frequency: '3 aplicaciones escalonadas', notes: 'Grupo frutales · dosis creciente' },
      { crop: 'zarzamora',     cropName: 'Zarzamora',       stages: ['crecimiento vegetativo', 'floración', 'fructificación'],dose: '1,5 – 2,5 L/200 L foliar · 2,0 – 3,0 L/ha riego', method: 'Foliar / Fertirrigación', frequency: '3 aplicaciones escalonadas', notes: 'Grupo frutales · dosis creciente' },
      { crop: 'aguacate',      cropName: 'Aguacate',        stages: ['brotación', 'floración', 'cuajado'],                    dose: '1,5 – 2,5 L/200 L foliar · 2,0 – 3,0 L/ha riego', method: 'Foliar / Fertirrigación', frequency: '3 aplicaciones escalonadas', notes: 'Grupo frutales · dosis creciente' },
      { crop: 'citricos',      cropName: 'Cítricos',        stages: ['brotación', 'floración', 'cuajado'],                    dose: '1,5 – 2,5 L/200 L foliar · 2,0 – 3,0 L/ha riego', method: 'Foliar / Fertirrigación', frequency: '3 aplicaciones escalonadas', notes: 'Grupo frutales · dosis creciente' },
      { crop: 'maiz',          cropName: 'Maíz',            stages: ['crecimiento vegetativo', 'floración'],                  dose: '0,5 – 1,5 L/200 L foliar · 1,0 – 2,0 L/ha riego', method: 'Foliar / Fertirrigación', frequency: '3 aplicaciones escalonadas', notes: 'Grupo cereales · dosis creciente' },
      { crop: 'cana-azucar',   cropName: 'Caña de Azúcar',  stages: ['crecimiento vegetativo', 'macollamiento'],              dose: '0,5 – 1,5 L/200 L foliar · 1,0 – 2,0 L/ha riego', method: 'Foliar / Fertirrigación', frequency: '3 aplicaciones escalonadas', notes: 'Grupo cereales/caña · dosis creciente' },
      { crop: 'frijol',        cropName: 'Frijol',          stages: ['crecimiento vegetativo', 'floración'],                  dose: '0,5 – 1,5 L/200 L foliar · 1,0 – 2,0 L/ha riego', method: 'Foliar / Fertirrigación', frequency: '3 aplicaciones escalonadas', notes: 'Grupo leguminosas · dosis creciente' },
    ],
    documents: {
      datasheet: '/docs/fichas-tecnicas/brotanic.pdf',
    },
  },
]

// ─── Export final ─────────────────────────────────────────────────────────

export const PRODUCTS: Product[] = [
  ...ORGANICOS,
  ...ESPECIALIDADES,
  ...BIOESTIMULANTES,
  ...NUTRICION,
  ...BIOPROTECCION,
  ...AGROBIONSA,
  ...VEGANIC,
]

// ─── Helpers ──────────────────────────────────────────────────────────────

export function getProductBySlug(slug: string): Product | undefined {
  return PRODUCTS.find((p) => p.slug === slug)
}

export function getProductsByLine(line: Product['line']): Product[] {
  return PRODUCTS.filter((p) => p.line === line)
}

export function getProductsByBrand(brand: Product['brand']): Product[] {
  return PRODUCTS.filter((p) => p.brand === brand)
}

export function getFeaturedProducts(): Product[] {
  return PRODUCTS.filter((p) => p.featured)
}

export function getRelatedProducts(product: Product, limit = 4): Product[] {
  return PRODUCTS
    .filter((p) => p.id !== product.id && (p.line === product.line || p.crops.some((c) => product.crops.includes(c))))
    .slice(0, limit)
}

export function searchProducts(query: string): Product[] {
  const q = query.toLowerCase()
  return PRODUCTS.filter(
    (p) =>
      p.name.toLowerCase().includes(q) ||
      p.description.toLowerCase().includes(q) ||
      p.solves_problems.some((s) => s.toLowerCase().includes(q)) ||
      p.category.toLowerCase().includes(q),
  )
}
